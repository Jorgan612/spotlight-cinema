...
const MovieCard  = ({img, id, getSingleMovieDetails}) => {
  ...
  return (
    <div className='movie-card' onClick={() => getSingleMovieDetails(id)}>
      <Link to='/moviedetails'><img className='poster-img' id={id} src={`https://image.tmdb.org/t/p/w500${img}`} alt='movie poster'/></Link>
    </div>
  )
}
